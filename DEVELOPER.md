...
This project is build from the [java webapp template](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/poc-backend-template/browse)
...
where "poc-backend" is your artifactID from above.

The Swagger UI is available at <http://localhost:8080/poc-backend/api/>, providing access to the `v1` version of the GUI. 
...
During development the configurations are located in the `conf`-folder as `conf/poc-backend-behaviour.yaml`,  
`conf/poc-backend-environment.yaml` and `conf/poc-backend-local.yaml`. 
...
repositories. To guard against this, `conf/poc-backend-environment.yaml` is added to `.gitignore`. 
...
should
  ...
  * Add the path to the new OpenAPI YAML to `urls` in `webapp/api/index.html`. The first entry in the array is the
    default when visiting the main API-page at <http://localhost:8080/poc-backend/api/>  
...
```
[Unreleased](https://github.com/kb-dk/poc-backend-template/compare/v1.0.0...HEAD)
[1.1.0](https://github.com/kb-dk/poc-backend-template/compare/v1.0.0...v1.1.0)
[1.0.0](https://github.com/kb-dk/poc-backend-template/releases/tag/v1.0.0)
```

For BitBucket (KB's internal git), the syntax is
```
[Unreleased](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/poc-backend-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.1.0&sourceBranch=refs%2Fheads%2Fmaster)
[1.1.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/poc-backend-template/compare/commits?targetBranch=refs%2Ftags%2Fv1.0.0&sourceBranch=refs%2Ftags%2Fv1.1.0)
[1.0.0](https://sbprojects.statsbiblioteket.dk/stash/projects/ARK/repos/poc-backend-template/commits?until=refs%2Ftags%2Fv1.0.0)
```
Note that both the `ARK`-part and the repo-id `poc-backend-template` is project-specific.


## Release procedure

1. Review that the `version` in `pom.xml` is fitting. `poc-backend` uses
...
which boils down to
   ...
   * Run `git push`   
   ...
   `mvn release:rollback ; git tag -d poc-backend-1.4.2 ; git push --delete origin poc-backend-1.4.2`
